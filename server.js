...
async function initDb() {
  pool = mysql.createPool({
    ...
    charset: "utf8mb4",
    ssl: {
      rejectUnauthorized: false, // ⚠️ TEMPORARY FIX for Aiven self-signed cert
    },
  });
...
// ➤ GET /admin (Basic Auth Protected)
    ...
    const htmlRows = rows
      .map(
        ...
        <tr>
          ...
          <td style="white-space:pre-wrap;max-width:420px">${escapeHtml(
            r.message
          )}</td>
          <td style="white-space:pre-wrap;max-width:300px">${escapeHtml(
            r.extra
          )}</td>
        </tr>`
      )
...
<body>
  <div class="header">
    <h1>SVRX Suggestions Dashboard</h1>
    <button onclick="location.reload()">Refresh</button>
  </div>
  <table>
    <thead><tr><th>ID</th><th>Time</th><th>Type</th><th>Priority</th><th>Name</th><th>Email</th><th>Message</th><th>Extra</th></tr></thead>
